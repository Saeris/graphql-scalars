...
const validate = value => {
  ...
  Joi.assert(parsed, Joi.number().positive().allow(0), new TypeError(`Value is not a positive number: ${parsed}`))
  return parsed
}
...
export const UnsignedInt = new GraphQLScalarType({
  ...
  parseLiteral({ kind, value }) {
    if (kind !== Kind.INT) {
      throw new GraphQLError(`Can only validate integers as non-negative integers but got a: ${kind}`)
    }
