...
const validate = value => {
  ...
  Joi.assert(value, Joi.string().ip({ version: `ipv6` }), new TypeError(`Value is not a valid IPv6 address: ${value}`))
  return value
}
...
export const IPv6 = new GraphQLScalarType({
  ...
  parseLiteral({ kind, value }) {
    if (kind !== Kind.STRING) {
      throw new GraphQLError(`Can only validate strings as IPv6 addresses but got a: ${kind}`)
    }
