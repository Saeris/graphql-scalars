...
const validate = value => {
  ...
  Joi.assert(parsed, Joi.number().port(), new TypeError(`Value is not a valid TCP port: ${parsed}`))
  return parsed
}
...
export const Port = new GraphQLScalarType({
  ...
  parseLiteral({ kind, value }) {
    if (kind !== Kind.INT) {
      throw new GraphQLError(`Can only validate integers as TCP ports but got a: ${kind}`)
    }
